    ...
    if (old_it != _old_items.end())
    {
      ...
      auto new_node = std::make_shared<Node>(
            Node{
              _goal,
              0.0,
              current_cost + remaining_cost.value(), //对于重复进入的点，说明该点有绕路的现象，很难是最短的路径，所以增加它的当前cost来绕开这条路径
              top
            });
...
{
  auto heuristic = _heuristic->get(); // CacheManagerPtr<EuclideanHeuristic> _heuristic;
  ...
  auto node = solution;
  while (node) // 生成一条最短路径，key到goal， new_items则是记录了该条路径的所有waypoints，及每个点对应的cost
  {
