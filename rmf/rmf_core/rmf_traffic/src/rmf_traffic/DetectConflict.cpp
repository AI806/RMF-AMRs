...
BoundingProfile get_bounding_profile(
  ...
  //get_characteristic_length相当于是是一个固定的偏移
{
...
{
  Profile::Implementation output = Profile::Implementation::get(profile); //返回profile的implementation类的地址
  if (!output.vicinity)
  ...
  const Time time)
{
  ...
  for (const auto& pair : pairs) //调用fcl库函数来判断两个区域是否重叠
  {
  ...
  const Trajectory::const_iterator& b_it)
{
  ...
  const auto start_time =
    std::max(spline_a.start_time(), spline_b.start_time()); //判断两条路径的起始时间
    ...
    std::vector<DetectConflict::Implementation::Conflict>* output_conflicts)
{
  ...
  if (output_conflicts) //为何要清除已有的碰撞列表？
...
//==============================================================================
Trajectory slice_trajectory( //截取部分轨迹点
    const Time start_time,
    const Spline& spline,
    ...
    std::vector<DetectConflict::Implementation::Conflict>* output_conflicts)
{
  ...
  {
    if (!spline_a)
      spline_a = Spline(a_it); // 这里才是对trajectory相关参数进行初始化， 这里不是处理轨迹，而是处理两点之间的轨迹拟合。也就是说spline类处理的是两点。trajectory处理的是整条轨迹
    ...
    if (D.initially_approaching()) //初始点就在靠近，则认为他们是会相撞
    {
    ...
    {
      if (!check_overlap(profile_a, *spline_a, profile_b, *spline_b, t))//如果不重叠了，则用detect_invasion来判断是否碰撞
      {
    ...
    const bool still_close = check_overlap(
          profile_a, *spline_a, profile_b, *spline_b, D.finish_time()); //到spline轨迹终点，在判断一次是否重叠

    if (spline_a->finish_time() < spline_b->finish_time())//切换到下一点
    {
      spline_a = rmf_utils::nullopt;
      ++a_it;
    }
    ...
    if (!still_close)//如果不重叠，则切换到detect_invasion
    {
      return detect_invasion(
  ...
  std::vector<Conflict>* output_conflicts)
{
  ...
  // 如果vicinity没有数据，则将它设置为footprint的值。理解：footprint指机器人的位置区域，vicinity指机器人的周边区域
  ...
  // 判断两个轨迹在时间上是否重叠，即a结束b还未开始，或者a开始b还未结束
  if (!have_time_overlap(trajectory_a, trajectory_b))
    return rmf_utils::nullopt;

  Trajectory::const_iterator a_it;
  Trajectory::const_iterator b_it;
  // 找到时间同步点，即a，b会在相同时间到达的点
  // 需要找到trajectory在哪儿进行初始化，比如parameters的初始化
