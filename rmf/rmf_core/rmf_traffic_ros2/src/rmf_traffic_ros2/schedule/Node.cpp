  ...
  active_conflicts(database)
{
  ...
  try
  {
    auto participant_logger = std::make_unique<YamlLogger>(log_file_name);// totally own the pointer and will not share with others, cannot copy, move only
  ...
  register_query_service =
    ...
    { this->register_query(request_header, request, response); }); // here lambda function is the defined callback 
  ...
  conflict_check_thread = std::thread(
    [&]()
    {
      ...
      while (rclcpp::ok(get_node_options().context()) && !conflict_check_quit) // ros的主循环
      {
        ...
        {
          std::unique_lock<std::mutex> lock(database_mutex);// conflict_check_cv.wait_for等待获取database_mutex锁的解除
          conflict_check_cv.wait_for(lock, std::chrono::milliseconds(100), [&]() 
          {
            return (database->latest_version() > last_checked_version)
            && !conflict_check_quit;
          }); // 这个wait_for会一直等待，直到database->latest_version() > last_checked_version 或者时间超过100ms
