...
class TranslationHeuristic // 继承了一个标准的unordered_map类
...
private:
  std::shared_ptr<const Supergraph> _graph; // 重置继承类中的_graph
  ShortestPathHeuristicCacheMap _heuristic_cache;
};

// using ShortestPathHeuristicCacheMap = CacheManagerMap<ShortestPathHeuristicFactory>;
