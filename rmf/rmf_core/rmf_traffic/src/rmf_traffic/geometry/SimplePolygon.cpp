  ...
  const SimplePolygon::Intersections& intersections)
{
  ...
  // IntersectingPair是一个vector,它包含每对曲线的节点及位置信息，可以通俗的理解为每对包含两条曲线
  ...
  const std::vector<Eigen::Vector2d>& polygon)
{
  ...
  subpolygon_queue.emplace_back( //为何要做这一步？ 将原始的数据移到新建的subpolygon_queue中
    [&polygon]() -> Subpolygon
    {
      Subpolygon sp;
      for (std::size_t i = 0; i < polygon.size(); ++i)
  ...
  while (!subpolygon_queue.empty())
  {
    ...
    // point to the triangle's pivot point. (中心点)
