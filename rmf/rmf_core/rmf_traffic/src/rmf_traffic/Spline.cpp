  ...
  const Eigen::Vector3d& v1)
{
  ...
  for (int i = 0; i < 3; ++i) //根据速度位置解一个三次方程，这里将起始点时间当做0，终点时间当做1
  {
    // *INDENT-OFF*
...
Spline::Parameters compute_parameters(
  const Trajectory::const_iterator& finish_it) //using const_iterator = base_iterator<const Waypoint>;

{
  const Trajectory::const_iterator start_it =
    --Trajectory::const_iterator(finish_it); // 这里是将起始点设置为终点的前一个点？ 因为每次只考虑一个节点，然后用spline来拟合两个节点间的连接
  ...
  const double delta_t = compute_delta_t(finish_time, start_time); //计算两点之间的时间间隔
  ...
  return {
    compute_coefficients(x0, x1, v0, v1), // coeffs - 4 x 3, 这里3表示的是坐标维度， 比如x,y,z
    delta_t,
  ...
  const internal::WaypointList::const_iterator& finish_it)
{
  const internal::WaypointList::const_iterator start_it =
    --internal::WaypointList::const_iterator(finish_it); //拿到前一个点的iterator
...
//==============================================================================
double compute_scaled_time(const Time& time, const Spline::Parameters& params) //计算两个相对时间，并归一化到【0，1】之间。
{
...
//==============================================================================
Eigen::Vector3d compute_position(//直接根据spline方程f(t)计算 位置
...
//==============================================================================
Eigen::Vector3d compute_velocity(//spline方程f(t)的一阶导
  const Spline::Parameters& params,
  const double time)
{
  Eigen::Vector3d result = Eigen::Vector3d::Zero();
...
//==============================================================================
Eigen::Vector3d compute_acceleration(//二阶导
  const Spline::Parameters& params,
  const double time)
{
  Eigen::Vector3d result = Eigen::Vector3d::Zero();
...
std::array<Eigen::Vector4d, 3> normalize_coefficients(
    ...
    const Spline& spline)
{
  // 在计算位置和速度时，时间已经归一化为【0，1】
  const Eigen::Vector3d x0 = spline.compute_position(t0);
  const Eigen::Vector3d x1 = spline.compute_position(t1);
  const Eigen::Vector3d v0 = delta_t * spline.compute_velocity(t0); // 这个函数是除了个总时间，所以这里要乘回来
...
{
  const double derivative = compute_derivative(0.0, _params); //计算的是t0时刻
  if (derivative < 0.0) // 小于0，说明速度和位置是相向的
    return true;
