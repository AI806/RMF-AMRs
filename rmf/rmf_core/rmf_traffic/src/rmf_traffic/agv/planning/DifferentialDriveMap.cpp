    ...
    std::vector<std::string> maps)
{
  ...
  const double minimal_cost =
      rmf_traffic::time::to_seconds(trajectory.duration()); //没有旋转，只有直走
  ...
  std::vector<std::string> maps)
{
      ...
      -> DifferentialDriveMapTypes::RouteInfo
  {
    ...
    const auto finish_time = *trajectory.finish_time();
    const auto finish_yaw = trajectory.back().position()[2];
    return {finish_time, finish_yaw, std::move(routes)}; // 构造一个DifferentialDriveMapTypes::RouteInfo
  };

  // 第一层， 也是最外层函数，只有一个参数 child_yaw， 返回类型为DifferentialDriveMapTypes::RouteFactory 函数
  return [factory = std::move(factory)]( 
      std::optional<double> child_yaw) 
      -> DifferentialDriveMapTypes::RouteFactory 
  {
    // 第二层， 两个参数 start_time， initial_yaw 返回类型为DifferentialDriveMapTypes::RouteInfo的构造函数
    return [factory, child_yaw](
        const rmf_traffic::Time start_time,
        const double initial_yaw) -> DifferentialDriveMapTypes::RouteInfo
    {
      // 第三层，三个参数 start_time, initial_yaw, child_yaw, 返回类型为DifferentialDriveMapTypes::RouteInfo的对象
      return factory(start_time, initial_yaw, child_yaw);
    };
  };
}
