...
public:
  ...
  static Waypoint make(Args&& ... args)
  {
    ...
    result._pimpl = rmf_utils::make_impl<Implementation>(
      Implementation{std::forward<Args>(args)...}); // 同时也构造了waypoint的implementation的对象
...
public:
  ...
  std::vector<double> orientations; // 应该是对点的方向约束，定义了哪些方向可以进入到该点

  bool apply(Eigen::Vector3d& position,
    const Eigen::Vector2d& /*course_vector*/) const final // 找到需要旋转最小的角度的方向作为入口方向
  {
...
//==============================================================================
std::size_t Graph::Lane::capacity() const
{
  ...
  Eigen::Vector2d location) -> Waypoint&
{
  _pimpl->waypoints.emplace_back(
    Waypoint::Implementation::make( // 注意，这里不是初始化implementation，而是调用implementation的一个成员函数
      _pimpl->waypoints.size(),
      std::move(map_name), std::move(location))); // 所以这里只是初始化了waypoint的implementation的前三个变量，其余的使用默认值。
...
bool Graph::add_key(const std::string& key, std::size_t wp_index)
{
  ...
  // waypoint 是个类，get返回的这个类指向implementation的地址
...
auto Graph::add_lane(
    ...
    const Lane::Node& exit, const std::size_t cap) -> Lane&
{
