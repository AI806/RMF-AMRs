...
public:
  ...
  iterator lower_bound(const Key& k) //Storage = std::vector<Element>; 这里第一个参数_storage.begin()返回的是Element的迭代器
  {
  ...
  template <typename... Args>
  iterator emplace_hint(iterator hint, const Key& key, Args&&... args) // 根据拿到的提示，将数据插入到storage中
  {
    if (_storage.empty())
    {
  ...
  void rotate(iterator mover, iterator target)
  {
    ...
    if (mover < target) // 这里只是将两个指针的位置交换
    {
      std::rotate(mover, mover+1, target); //一个序列的逆时针旋转平移， rotate is happened between [mover, target], mover+1 is the to-be-rotated value
    }
    else
    {
      assert(target < mover);
