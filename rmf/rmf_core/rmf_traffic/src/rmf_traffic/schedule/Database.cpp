...
public:
  ...
  struct ParticipantState
  {
    ...
    RouteId last_route_id = std::numeric_limits<RouteId>::max(); // 这里返回的是uint64_t的最大值
  };
    ...
    const Input& input)
  {
    ...
    {
      const auto insertion = storage.insert({item.id, RouteStorage()}); //返回{value, bool}
  ...
  void insert_items(
    ...
    const Input& input)
  {
    assert(entries.size() == input.size());// 这一步是在check_route_ids验证后，如果有的ID已经被占用，则entries和input的size会不一致

    for (std::size_t i = 0; i < input.size(); ++i)
    {
      const auto& item = input[i];
      const RouteId route_id = item.id;
      if (rmf_utils::modular(state.last_route_id).less_than(route_id)) //可以理解为普通的大小比较
    ...
    const std::function<bool(const RouteEntry&)>& relevant) final
  {
    ...
    if (last && last->route && relevant(*last))
    {
      ...
      if (newest->route && relevant(*newest))
      {
        ...
        //changes 是 std::unordered_map<ParticipantId, ParticipantChanges>， newest->participant 是 ParticipantId
        //unordered_map可以直接往map中添加数据， 如果map中的ID没有对应的数据，则会拿到一个默认的ParticipantChanges（初始化为0）
...
auto Database::changes(
  ...
  rmf_utils::optional<Version> after) const -> Patch // Version - uint64_t
{
  std::unordered_map<ParticipantId, ParticipantChanges> changes; // ParticipantId - unint64_t。
  if (after) //存在
  {
