...
public:
  ...
  void expand(const NodePtr& top, SearchQueue& queue)
  {
    ...
    { // current_wp_index 不在 _old_items 中
      ...
      auto new_node = std::make_shared<Node>(
            Node{
              _goal,
              0.0, // remaining_cost_estimate
              current_cost + remaining_cost.value(), // current_cost
              top // parent
            });
...
private:
  ...
  double _max_speed;
  const EuclideanHeuristic::Storage& _old_items; // unorder_map, 成员是ID 和 cost值
    ...
    Storage& new_items) const
{
  ...
  if (start_map == *_goal_map) // 这里可以看出，EuclideanHeuristic主要用于多楼层优化， 单楼层优化时，不用考虑
  {
