...
public:
  ...
  bool is_finished(const SearchNodePtr& top) const
  {
    ...
    if (info.waypoint == _goal_waypoint)
    {
      ...
      if (std::abs(angle_diff) <= _interpolate.rotation_thresh) // 位置和角度都满足到达要求才算是到达目的地
        return true;
    }
  ...
  void expand(const SearchNodePtr& top, SearchQueue& queue)
  {
    ...
    if (!_visited.insert(current_entry.value()).second) // 把当前的entry点设置为已访问
    {
    ...
    if (current_wp_index == _goal_waypoint)
    {
      ...
      assert(_goal_yaw.has_value()); // 如果位置到达了，但方向还没有到达，则只调整方向
  ...
  Storage& new_items) const -> SolutionNodePtr
{
  ...
  //_heuristic_map.get(goal_waypoint_index)： CacheManager<Cache<TranslationHeuristicFactory::Generator>>
  // _heuristic_map它是CacheManagerMap类的对象，
  // ->get() 返回的是Cache<TranslationHeuristicFactory::Generator> = Cache<TranslationHeuristic>
  // CacheManagerMap<TranslationHeuristicFactory> _heuristic_map;
  auto heuristic = _heuristic_map.get(goal_waypoint_index)->get();

  // ->get() 调用 TranslationHeuristic::generate 生成新的数据
  ...
  const double yaw) const
{
  ...
  if (start_waypoint == _goal_waypoint) // 已经到达目标点
  {
    if (!_goal_yaw.has_value()) //检查是否有角度要求
    {
...
// 这个函数是为了找到初始点和终点之间的键值对，这里的终点是需要考虑与之相连的lane的信息，所以才有这个函数的存在
