...
struct Traversal
{
  ...
  std::array<std::optional<Alternative>, 3> alternatives; // 这里3 表示三种情况，前进，倒着走，和任意方向
};
using Traversals = std::vector<Traversal>; 
using ConstTraversalsPtr = std::shared_ptr<const Traversals>;

//==============================================================================
class TraversalGenerator //简单来说，TraversalGenerator就是记录一条一条的轨迹
...
public:
  ...
  private:
    ...
    std::map<double, Entry> _angled_entries; // 有入场角度约束， 个人理解： 这个入场角度即进入该waypoint时所在的lane的角度

    // If there are any entries that don't require a specific entry angle, then
    // that will be captured here.
    std::optional<Entry> _agnostic_entry; // 没有入场角度约束
