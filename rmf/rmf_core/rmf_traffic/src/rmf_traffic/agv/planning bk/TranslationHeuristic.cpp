...
public:
  ...
  void expand(const NodePtr& top, SearchQueue& queue)
  {
    ...
    if (old_it != _old_items.end()) // current_wp_index 已经在_old_items 中
    {
    ...
    const auto traversals = _graph->traversals_from(current_wp_index); // traversals_from记录的是所有点到点之间的最佳路径和时间
    assert(traversals);
...
{
  auto heuristic = _heuristic->get(); // CacheManagerPtr<ShortestPathHeuristic> _heuristic;
  ...
  queue.push(
    std::make_shared<TranslationExpander::Node>(
      TranslationExpander::Node{
        key,
        start_heuristic.value(), // 在lane上以最快速度直线travel到终点的cost
        0.0,
        nullptr
      }));
