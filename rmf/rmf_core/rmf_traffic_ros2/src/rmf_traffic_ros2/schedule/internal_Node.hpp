...
public:
  ...
  using Version = rmf_traffic::schedule::Version; // Version is uint64_t
  using ItineraryVersion = rmf_traffic::schedule::ItineraryVersion; // using ItineraryVersion = uint64_t
  using ParticipantId = rmf_traffic::schedule::ParticipantId; // PartticipantId is uint64_t
  ...
  public:
    ...
    struct Wait
    {
      Version negotiation_version; // uint64_t
      rmf_utils::optional<ItineraryVersion> itinerary_update_version; // uint64_t
    };
    ...
    {
      ConflictSet add_to_negotiation; // add_to_negotiation is defined as std::unordered_set<uint64_t>
      const Version* existing_negotiation = nullptr;
      for (const auto c : conflicts)
      {
        const auto wait_it = _waiting.find(c); // if not find, return unordered_map::end
        if (wait_it != _waiting.end()) // mean that the conflict is already in the unordered_set
        {
      ...
      if (add_to_negotiation.empty()) // seem this "sentence" is no meaning
        return rmf_utils::nullopt;
   ...
   // put the newly detected conflicts into waiting list using the participant ID
    void conclude(const Version version)
    {
      ...
      for (const auto p : participants)
      {
        const auto insertion =
          _waiting.insert({p, Wait{version, rmf_utils::nullopt}}); // pair<iterator, bool> unordered_map::insert(...)
      ...
      const rmf_utils::optional<ItineraryVersion> update_version)
    {
      ...
      if (update_version) // no change, why? Just for checking?
