...
private:
  ...
  void publish_map_markers()
  {
    ...
    auto make_lane_label =
      [&](const GraphNode& start, GraphNode& end, const uint64_t count, const uint8_t capacity) -> Marker
      {
        Marker label_marker;
        label_marker.header.frame_id = _frame_id; // map
        label_marker.header.stamp = rmf_traffic_ros2::convert(
        ...
        // Set the pose of the marker
        label_marker.pose.orientation.w = 1;
        label_marker.pose.position.x = (start.x + end.x) / 2.0;
        label_marker.pose.position.y = (start.y + end.y) / 2.0;
        label_marker.pose.position.z = 0.0;
        ...
        // Set the text of the marker
        label_marker.text = std::to_string(capacity);
        // Set the color of the marker
        label_marker.color = make_color(1, 1, 1);
    ...
    else
    {
      ...
      for (const auto nav_graph : _level.nav_graphs)
      {
        ...
        for (const auto edge : nav_graph.edges)
        {
          ...
          marker_array.markers.push_back(make_lane_label(n1, n2, waypoint_count, edge.capacity));
          ++waypoint_count;
        }
        graph_count++;
