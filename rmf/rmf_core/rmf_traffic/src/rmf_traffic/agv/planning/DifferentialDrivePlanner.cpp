...
public:
  ...
  bool quit(const SearchNodePtr& top, SearchQueue& queue) const
  {
    ...
    {
      if (*_saturation_limit < _internal->popped_count + queue.size()) // 如果队列的大小超出了允许的搜索深度， 则退出
        return true;
    }
    ...
    if (_maximum_cost_estimate.has_value())
    {
      ...
      if (*_maximum_cost_estimate < cost_estimate) // 如果头部节点的cost超出允许的最大cost，则退出
        return true;
    }

    if (_interrupter && _interrupter()) // 收到终端信息
    {
  ...
  bool is_at_goal(const SearchNodePtr& top) const
  {
    ...
    {
      if (!_goal_yaw.has_value()) // 是否需要考虑角度
        return true;

      const double angle_diff = rmf_utils::wrap_to_pi(top->yaw - *_goal_yaw);
      if (std::abs(angle_diff) <= _rotation_threshold) // 在角度允许范围内，才认定为到达终点
        return true;
    }
  ...
  bool is_finished(const SearchNodePtr& top) const
  {
    ...
    {
      if (top->time < *_goal_time) // 如果头部节点记录的时间小于设置的goal时间，则继续搜索
        return false;
    }
  ...
  {
    const auto& start = top->start.value(); // Planner::Start
    const std::size_t target_waypoint_index = start.waypoint(); // 在graph中的waypoint的索引

    // _supergraph->original()返回 Graph::Waypoint 对象的vector，也就是所有Waypoint的一个队列
    const auto& wp = _supergraph->original().waypoints[target_waypoint_index]; //Graph::Waypoint的对象，在graph中的一部分
    const Eigen::Vector2d wp_location = wp.get_location(); // 位置坐标

    // If this start node did not have a waypoint, then it must have a location
    assert(start.location().has_value());// start.location() 返回的是robot的初始坐标。 这里必须有值，因为这是机器人的当前坐标

    const auto approach_info = make_start_approach_trajectories( // 从robot当前点到graph中最近点进行位置和角度插值，初始化轨迹
    ...
    if (const auto lane_index = start.lane()) // 存在lane的索引
    {
      const auto& lane = _supergraph->original().lanes[*lane_index];  // 返回lane的信息

      const std::size_t wp0_index = lane.entry().waypoint_index(); //lane入口点索引
      const auto& wp0 = _supergraph->original().waypoints[wp0_index]; //lane入口点坐标
      const auto& wp0_map = wp0.get_map_name(); // name of the map

      assert(lane.exit().waypoint_index() == target_waypoint_index); // 确保lane出口点是Start的位置索引。。。
      ...
      if (lane.exit().event()) // lane中存在事件， 比如开门，进电梯等
      {
    ...
    for (const auto& approach_trajectory : approach_info.trajectories) // 插值出的多条trajectory
    {
    ...
    const double cost_factor) const
  {
    ...
    if (_supergraph->original().waypoints[wp_index].is_passthrough_point()) //当前点不能通过
      return nullptr;
    ...
    Route route{map_name, std::move(trajectory)}; // route 在 trajectory的基础上添加了map name
      ...
      SearchQueue& queue) const
  {
    ...
    const auto keys = _supergraph->keys_for(
      top->waypoint.value(), _goal_waypoint, _goal_yaw); // 为何要用目标点的信息？？？？
...
//      std::cout << "Free solution: ";
  ...
  // 从robot当前点到graph中最近点进行位置和角度插值，初始化轨迹
    ...
    const double hold_time) const
  {
    ...
    // 初始化时， start中的location应该是机器人所在的实际位置点，它可能不在graph中， 为waypoint则是location在graph中的索引， 它对应的点是graph中的位置点
    ...
    DifferentialDriveConstraint constraint{
      differential->get_forward(), // robot的方向向量？
      differential->is_reversible() // 使用车的方向来初始化运动约束
    };

    const std::size_t waypoint_index = start.waypoint(); //这个waypoint返回的是它在graph的索引。

    const Eigen::Vector2d p0 = *location_opt;
    // original返回的是graph的implementation, 它的waypoint是Waypoint的一个对象，get_location返回的是它的
    const Eigen::Vector2d p1 =
        _supergraph->original().waypoints[waypoint_index].get_location(); 

    // options返回的是Internal::Options的Implementation
    const double translation_thresh = _supergraph->options().translation_thresh;

    const double dist = (p1 - p0).norm(); // 机器人实际点与graph中最近点的距离
    if (dist < translation_thresh) // 如果很近，则不需要生成相应的虚拟轨迹
    {
    ...
    for (const auto& yaw_opt : yaw_options)
    {
      ...
      if (!orientation_constraint_satisfied( // 某些角度是不能旋转的吗？？？
      ...
      trajectory.insert(start_time, p_start, zero);// waypoint的数据格式是WaypointElement::Data， 包含三个数据，时间， 位置， 速度

      const Eigen::Vector3d p_oriented{p0.x(), p0.y(), yaw};
      internal::interpolate_rotation( // 进行角度插值，从当前角度插值到目标角度
            trajectory, w_nom, alpha_nom, start_time, p_start, p_oriented,
            rotation_thresh);

      const Eigen::Vector3d p_arrived{p1.x(), p1.y(), yaw};
      // trajectory.finish_time() 返回的是trajectory中最后一个waypoint点的时间
      // 在两点间进行现行插值，包括匀加速，匀速，匀减速三个过程
  ...
  SearchNodePtr make_start_node(const Planner::Start& start) const
  {
    ...
    if (approach_info.need_approach)
    {
      ...
      for (const auto& approach : approach_info.trajectories) // trajectories - std::vector<Trajectory> 
      { // approach - Trajectory, approach.back() - segments.back().myself - Trajectory::Waypoint
        const double yaw = approach.back().position()[2]; // 路径节点末尾的朝向

        // approach.duration() 计算trajectory持续的总时长
  ...
  : _config(std::move(config))
{
  _supergraph = Supergraph::make(
        Graph::Implementation::get(_config.graph()), // _config.graph() 返回 Graph的对象， Graph::Implementation::get返回Graph::Implementation的指针
  ...
  Planner::Options options) const
{
  ...
  State state{
    ...
    Issues{},
    std::nullopt,
    rmf_utils::make_derived_impl<State::Internal, InternalState>() // 可不可以这么理解？？ make_derived_impl其实是对虚类的继承类配置构造函数和析构函数。
  };
  ...
  ScheduledDifferentialDriveExpander expander{
    state.internal.get(), // _internal
    state.issues, // _issues
    _supergraph, // _supergraph
    DifferentialDriveHeuristicAdapter{ // _heuristic
      _cache->get(),  // CacheManagerPtr<DifferentialDriveHeuristic> _cache;
      _supergraph, // _graph
      goal.waypoint(), // _goal_waypoint
      rmf_utils::pointer_to_opt(goal.orientation()) // _goal_yaw
    },
    goal, // _goal_waypoint
