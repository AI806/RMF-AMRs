...
public:
  ...
  class OrientationConstraint // OrientationConstraint 是个虚类，但它定义了两个不同的make函数来生成不同的实现类的对象
  {
  public:
    ...
    make(std::vector<double> acceptable_orientations); // 构造AcceptableOrientationConstraint的实例，在转换回OrientationConstraint类型
    ...
    /// or backward.
    static rmf_utils::clone_ptr<OrientationConstraint>
    make(Direction direction, const Eigen::Vector2d& forward_vector);// 构造DirectionConstraint的实例，在转换回OrientationConstraint类型
  ...
  Lane& add_lane(
    ...
    const Lane::Node& exit, const std::size_t cap);
